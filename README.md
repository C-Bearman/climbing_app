...
- [ ] **Tooltip font rendering consistency** *(likely resolved by the Session Rating x-axis change above — worth a visual re-check, not yet confirmed)* — previously Session Rating and Flexibility Rating charts used a custom linear x-axis while Workout Progress used a standard category axis, and Chart.js rendered Canvas tooltip text slightly differently between the two. Now that all three chart types share the same category axis, this inconsistency should no longer occur, but hasn't been visually verified side-by-side yet.
